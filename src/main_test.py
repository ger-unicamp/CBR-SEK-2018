...
def manobra1():
    ...
    if(colors[SensorCorDir.value()] in cor_invalid): #confere se o sensor direito está fora da pista
        ...
        '''
            ...
            motorDireita.run_forever(speed_sp=0)
        '''
    elif(colors[SensorCorEsq.value()] in cor_invalid) or (colors[SensorCorDir.value()] in cor_invalid): #Tudo igual de maneira antagonica
        motorEsquerda.run_forever(speed_sp=0) #STOP ACTION
...
def main():
    ...
    while not btn.any():
        ...
        elif (colors[SensorCorDir.value()] != 'white') or (colors[SensorCorEsq.value()] != 'white') and not plaza: 
        ...
        '''
            ...
            motorDireita.stop(time_sp=700, speed_sp=200)
            motorEsquerda.stop(time_sp=700, speed_sp=200)
            x = 
            motorEsquerda.stop(stop_action="hold")
            if(SensorCorDir.value() == 'white'):
                while colors[SensorCorEsq.value()] != 'white':
                    motorEsquerda.run_forever(speed_sp=180)
                motorDireita.run_timed(time_sp=1100, speed_sp=-200) #Retorna ao meio da pista
                motorEsquerda.run_timed(time_sp=1100, speed_sp=-200)
                girarRobo(-90)
            elif SensorCorEsq.value() == 'white':
